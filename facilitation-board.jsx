...
// ===== 순서형 가이드 코치마크 (기존 로직/레이아웃과 분리된 오버레이 레이어) =====
// "다시 보지 않기" 상태는 그 사람 브라우저에만 저장돼야 하므로 localStorage에 보관한다
// (공유 저장소인 window.storage는 참여자 전원이 공유하므로 개인별 안내 이력에는 부적합).
const GUIDE_STORAGE_KEY = "onalign-guide-seen";

// 각 단계는 실제 UI 요소(data-guide 속성)를 가리킨다.
// screen: 이 단계가 뜨는 화면 / target: data-guide 값 / minNotes: 최소 포스트잇 수 조건 / once: 최초 1회만
const GUIDE_STEPS = [
  { id: "new-project", target: "new-project", screen: "projects", text: "여기서 프로젝트를 만들어보세요" },
  { id: "name-input", target: "name-input", screen: "name", text: "이름을 적으면 색이 자동으로 정해져요" },
  { id: "add-note", target: "add-note", screen: "opinion", text: "포스트잇을 만들고 자유롭게 적어보세요" },
  { id: "merge", target: "merge", screen: "opinion", minNotes: 2, text: "비슷한 의견은 합쳐보세요" },
  { id: "promote", target: "promote", screen: "opinion", minNotes: 1, text: "중요한 의견은 문제로 등록하세요" },
  { id: "problem-tab", target: "problem-area", screen: "problem", text: "여기서 문제 목록을 다듬으세요" },
  { id: "voting", target: "vote-area", screen: "voting", text: "팀이 가장 원하는 문제에 투표하세요" },
  { id: "save-image", target: "save-image", screen: "voting", once: true, text: "결과를 이미지로 남길 수 있어요" },
];
...
function GuideCoach({ screen, notesCount = 0 }) {
  const [seen, setSeen] = useState(loadSeenGuides);
  const [dismissed, setDismissed] = useState([]); // 이번 세션에 "다음"으로 넘긴 것 (영구 저장 안 함)
  const [dontShow, setDontShow] = useState(false);
  const [rect, setRect] = useState(null);

  // 현재 화면·조건을 만족하면서 아직 안 본, 그리고 이번 세션에 넘기지 않은 "첫" 단계 하나만 활성화
  // -> 여러 조건이 동시에 만족돼도 GUIDE_STEPS 정의 순서대로 하나씩만 노출된다 (큐 역할)
  const active = GUIDE_STEPS.find((s) => {
    if (s.screen !== screen) return false;
    if (seen.includes(s.id)) return false;
    if (dismissed.includes(s.id)) return false;
    if (s.minNotes && notesCount < s.minNotes) return false;
    return true;
  });
  const activeId = active ? active.id : null;

  // 대상 요소의 화면상 위치를 추적한다. 스크롤·레이아웃 변동·요소가 늦게 렌더되는 경우까지
  // 대응하기 위해 스크롤/리사이즈 이벤트 + 짧은 인터벌로 위치를 갱신하되, 변화가 있을 때만 상태를 바꾼다.
  useEffect(() => {
    setDontShow(false);
    if (!activeId) {
      setRect(null);
    ...
    const target = GUIDE_STEPS.find((s) => s.id === activeId).target;
    const update = () => {
  ...
  const finish = () => {
    // "다시 보지 않기" 체크 또는 once 단계면 localStorage에 영구 기록해 다시 안 뜨게 한다.
    if (dontShow || active.once) {
      const nextSeen = [...seen, active.id];
      setSeen(nextSeen);
      try {
        localStorage.setItem(GUIDE_STORAGE_KEY, JSON.stringify(nextSeen));
      } catch (e) {
        /* 저장 실패해도 진행에는 지장 없음 */
      }
    }
    setDismissed((d) => [...d, active.id]); // 다음 단계로 넘어감
  };

  // 툴팁은 대상 아래를 기본으로, 화면 하단에 가까우면 위쪽으로 띄운다.
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9998, pointerEvents: "none", fontFamily: "sans-serif" }}>
      {/* 전체를 덮는 모달이 아니라 pointerEvents:none 레이어라 뒤 화면은 그대로 조작 가능하다 */}
      ...
      <div
        style={{
          ...
          ...(below ? { top: rect.bottom + 14 } : { top: rect.top - 14, transform: "translate(-50%, -100%)" }),
          width: 244,
          background: "#242322",
          color: "#f2f1ec",
          borderRadius: 12,
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, position: "relative" }}>
          {active.once ? (
            <span style={{ fontSize: 11, color: "#8b877f" }}>한 번만 안내돼요</span>
          ) : (
            <label style={{ fontSize: 12, color: "#b6b2a9", display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
...
export default function FacilitationBoard() {
  ...
  const loadProjects = useCallback(async () => {
    try {
      const res = await window.storage.get(PROJECTS_INDEX_KEY, true);
  ...
  const createProject = async () => {
    ...
    await window.storage.set(PROJECTS_INDEX_KEY, JSON.stringify(nextList), true);
    await window.storage.set(boardKeyOf(project.id), JSON.stringify(emptyBoard()), true);
  ...
  const deleteProject = async (id) => {
    ...
    await window.storage.set(PROJECTS_INDEX_KEY, JSON.stringify(nextList), true);
    await window.storage.delete(boardKeyOf(id), true).catch(() => {});
    setProjects(nextList);
  };
  ...
  // shared=true 스토리지에서 현재 프로젝트의 보드 상태를 읽어옴
  const loadBoard = useCallback(async () => {
    if (!selectedProject) return;
    try {
      const res = await window.storage.get(boardKeyOf(selectedProject.id), true);
      if (res && res.value) {
  ...
  const saveBoard = useCallback(
    async (next) => {
      ...
      setBoard(next);
      try {
        await window.storage.set(boardKeyOf(selectedProject.id), JSON.stringify(next), true);
      } catch (e) {
  ...
  return (
    <div style={{ fontFamily: "sans-serif", maxWidth: 900, margin: "0 auto", padding: 16 }}>
      ...
      <GuideCoach screen={board.phase} notesCount={board.notes.length} />
    </div>
  );
}
