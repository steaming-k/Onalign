# Onalign 기획서 (Draft v16 — 회고 탭·문서 표준화 설계)
...
**결론: "회고" 탭이 5번째 탭으로 상시 존재하고, 참여자 각자 KPT를 작성하며, "완료" 시 문서 탭에 반영된다. (v16, 설계 구체화)**
...
3. "완료" 버튼: 회고 탭 안에서 작성 마감을 선언하는 액션 → 누르면 참여자 전원의 KPT 내용이 **"과정 문서"와 "결과 문서" 양쪽 모두에** 반영됨
...
| 질문 | 배경 |
|---|---|
| 우선순위 문제 자동 표시 + 해결여부 체크를 회고 탭에 유지할지 | 이전엔 "이게 없으면 차별점 소멸"이라고 판단했던 요소. 참여자별 KPT 입력창 **위쪽에 별도 섹션으로 유지**하는 걸 제안 — 확인 필요 |
| "완료" 버튼 권한 | 6-1의 리더 전용 원칙(팀 전체 문서에 영구 반영되는 되돌리기 어려운 액션)에 따르면 리더 전용이 자연스러워 보임 — 확인 필요 |
| "완료" 후 개별 KPT 재수정 가능 여부 | 잠가서 그 시점 스냅샷을 문서에 고정할지, 계속 편집 가능하게 두고 문서도 동기화할지 — 확인 필요 |
| 프로젝트 상태(`진행중`/`완료`) 필드를 별도로 둘지 | 이번 설명은 "회고 탭 완료 버튼"만 언급, 전체 프로젝트 상태 전환 개념은 없어 보임 — 단순화된 것으로 이해하고 진행 |
...
v15에서 미착수로 확인했던 항목이다. v16에서 참여자별 KPT 입력·문서 반영 방식으로 설계가 구체화됐다(5장, 5-1장 참조). 다만 "우선순위 자동표시 유지 여부", "완료 버튼 권한", "완료 후 재수정 가능 여부"는 아직 확인 필요 상태로 남아있다 — 구현은 그 확인 이후에 진행하는 게 안전하다.
