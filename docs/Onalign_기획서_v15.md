# Onalign 기획서 (Draft v15 — 실배포 명세서 대조 검증)
...
**결론: "프로젝트 완료" 버튼이 회고를 열되, 빈 캔버스가 아니라 시작 시점 우선순위에서 출발하게 한다.**

1. 프로젝트 상태: `진행 중` / `완료`
2. "프로젝트 완료" 버튼 → `회고` 탭 오픈
3. 회고 포맷: KPT (표준 포맷, 학습비용 최소화)
4. **핵심 차별화**: 회고 탭 상단에 아이데이션 우선순위 상위 문제 자동 표시 → 해결됨/부분해결/미해결 체크 → KPT 작성
5. 이미지 내보내기 확장: 아이데이션+회고 통합 타임라인 이미지
...
| Must — **미착수 (13-3 확인)** | "완료" 버튼 → KPT 회고 탭 | 신규, 실배포에서 아직 확인 안 됨 |
| Must — **미착수 (13-3 확인)** | 회고 탭 우선순위 자동 표시 + 해결여부 체크 | 신규, 없으면 차별점 소멸 |
...
**5장(아이데이션→회고 연결, MVP Must 항목)이 아직 구현되지 않은 것으로 보인다.** 명세서 어디에도 "회고", "KPT", "프로젝트 완료 버튼", "진행중/완료 상태" 관련 화면이 없다 — 지금 있는 탭은 의견작성/문제정리/우선순위결과/문서 4개뿐이다. 이건 4장 차별점 5번("아이데이션-회고 연결", 방어력 최대로 평가했던 항목)이자 7장 MVP Must 항목인데, 아직 착수 전인 것으로 보인다. 우선순위 재확인이 필요하다.
