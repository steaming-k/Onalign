# Onalign 기획서 (Draft v14 — 회의 녹음·텍스트화 기능 추가)
...
| Must | "완료" 버튼 → KPT 회고 탭 | 신규 |
| Must | 회고 탭 우선순위 자동 표시 + 해결여부 체크 | 신규, 없으면 차별점 소멸 |
...
| Should (15장 근거, 신규) | 회의 녹음 → 텍스트 변환(발화자 구분 없음) | 참여자 누구나 선택 가능. STT 외부 API 필요, 유료 전환 후보 |
...
**결론: Onalign으로 확정, 실배포 완료. 단 저장 방식과 "문서" 탭은 미확인 상태로 남음.**

- **서비스명**: Onalign, 배포 화면 타이틀에도 반영 확인
- **배포**: https://onalign.vercel.app — 6장 "프로토타입→실앱 전환"이 이미 시작됨
- ⚠️ **미확인 사항**
  - 데이터 저장 방식(localStorage vs 서버·DB) — window.storage 사용 불가 환경이라 확인 필요
  - "문서" 탭의 정체 — 5장 회고 화면인지 별도 기능인지 스크린샷만으로 판단 불가

**이번 라운드 UX 개선 7건** (상세는 Claude Code 프롬프트 참조)
- 상단 고정 로고 / 세션 재진입 시 이름 생략 / 빈 보드 삭제 / 병합모드 클릭 영역 확장
- 포스트잇 자동 줄바꿈 / "문제로" 상단 고정(화면 전환 없음) / 우선순위 투표 화면 내 처리

**설계 확정 사항**
- "문제로": 화면 전환 없이 보드 내 상단 고정. "문제 정리" 탭은 폐지 아니라 "여러 보드 통합 관리"로 역할 축소
- "우선순위 투표": 별도 화면 없이 카드에 직접 투표. 기존 탭은 "우선순위별 결과"(득표순 목록)로 역할 전환
- 11-3 절충안 반영 시 "투표 마감" 시점 명시 필요 — Claude Code에 확인 질문으로 전달
...
3. **회의 녹음 → 텍스트 변환 (신규)**: 참여자 누구나 개인 선택으로 녹음 시작/종료 가능. 발화자 구분 없이 STT(Whisper API 등)로 텍스트만 저장해 "문서" 탭에 "회의 원문"으로 추가
   - **목적 재확인**: 발화자 구분이 아니라 "문서 탭 요약 문구가 어떤 맥락에서 나왔는지" 되짚을 원본 자료 확보 — 1장 핵심 문제(의도 파악 실패)를 보강하는 방향으로 판단
   - **권한**: 프로젝트 참여자 누구나 시작 가능 (리더 전용 아님) — 6-1의 "리더 전용" 원칙(팀 전체에 영향을 주는 되돌리기 어려운 결정)과 달리, 개인이 켜고 끄는 선택적 기능이라 예외로 분류
   - **비용 구조**: STT는 분당 과금 → 6장 수익화 논의에 "유료 플랜 전용 기능" 후보로 연결
   - **인프라 영향**: Vercel 서버리스 함수 + 외부 STT API 필요 — 브라우저 내부 처리만으로 완결되던 기존 구조에서 처음으로 벗어나는 기능

→ 셋 다 7장 MVP 표에 Should로 반영 완료
