# Onalign 기획서 (Draft v25 — 구글 로그인 착수·권한 매트릭스)
...
### 무엇 (What) — 지금 만들고 있는 것
5~15인 소규모 팀을 위한, 로그인 없이 이름만으로 참여하는 퍼실리테이션 도구. 의견 작성 → 병합 → 문제 정리 → 우선순위 투표 → **회고(KPT)** → 문서화까지 한 프로젝트 안에서 이어진다. 메인 경쟁자는 EasyRetro/TeleRetro이며, 이들과 달리 "아이데이션과 회고를 잇는다"는 게 가장 방어력 있는 차별점이다(3, 4장).
...
### 아직 열려 있는 것
1. **5~8인 규모에서 동시 작성이 실제로 효과 있는가** — 학계도 결론 없음(11-1). 실사용 로그로만 판단 가능
2. **회고가 실제로 팀에 도움이 되는가** — 실증 연구 자체가 얇음(11-2). "회고를 잘하게" 보다 "완료 전환율"부터 확인하는 게 현실적

### 다음 액션 (지금 할 수 있는 것 순서대로)
1. 12장 설문 배포 (캠프 내부+외부 최소 15~20건)
2. 사전 판단 기준(12장)과 응답 대조, 유리하게 재해석하지 않기
3. 로그인은 테스팅이 얼추 끝난 뒤 착수
...
### 6-1. 리더/팀원 권한 분리 (v25 — 착수 시점, 매트릭스로 구체화)

로그인 착수와 함께 세부를 확정한다. UI 문구에 이미 "리더가" 라는 표현이 박혀 있던 항목(13-6)들을 실제 권한 체크로 승격하고, 그 외 항목은 이번에 새로 판단해 제안한다.

| 기능 | 권한 | 근거/상태 |
|---|---|---|
| 회의 녹음 시작/종료 | 참여자 누구나 | ✅ 기존 확정 (개인 선택 기능) |
| 회고 "완료" 처리 | 참여자 각자 자기 몫만 | ✅ 기존 확정 (5장) |
| STEP 안내 배너 편집 | **리더 전용** | ✅ UI 문구에 이미 "리더가" 명시(2차 명세서) — 실제 체크로 승격 |
| 1인당 투표권 수 조정 | **리더 전용** | ✅ UI 문구에 이미 "리더가" 명시(2차 명세서) — 실제 체크로 승격 |
| 프로젝트 삭제 | **리더 전용 (신규 제안)** | 되돌릴 수 없는 파괴적 액션 — 확인 필요 |
| 문서 표준 4필드 편집(목적/배경/추진방향/기대효과) | **참여자 누구나 (신규 제안)** | 팀 전체가 같이 다듬는 성격의 내용으로 판단 — 확인 필요 |
| 문제 정리 탭 문구 다듬기 | **참여자 누구나 (신규 제안)** | 지금까지 병합·문제표시 등과 같은 결로, 제한 안 두는 쪽이 일관적 — 확인 필요 |
| 포스트잇 작성/병합/문제표시/보류, 투표 참여, 프로젝트 목표 편집 | 참여자 누구나 (변경 없음) | 기존 그대로 |
| docx/마크다운 다운로드, 이미지 저장, 프롬프트 추출 | 참여자 누구나 (변경 없음) | 읽기·내보내기 성격, 제한 불필요 |

⚠️ **확인 필요 (음영 표시된 신규 제안 3개)**: 프로젝트 삭제, 문서 4필드 편집, 문제 정리 문구 편집 — 리더 전용으로 할지 참여자도 가능하게 할지. 표에 적은 제안대로 진행해도 되는지 확인해주시면 그대로 구현하고, 아니라면 어느 항목을 바꿀지 알려달라.
